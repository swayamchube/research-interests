...
\begin{proposition}
    Let $A\subseteq B$ be integral domains with $A$ integrally closed. Let $\alpha\in B$ be integral over an ideal $\fraka$ of $A$. Then $\alpha$, viewed as an element of $Q(B)\supseteq Q(A)$ is algebraic over the field of fractions $K$ of $A$. Further, if the minimal polynomial of $\alpha$ over $K$ is given by $x^n + a_{n - 1}x^{n - 1} + \cdots + a_0$, then each $a_i$ is an element of $\sqrt{\fraka}$.
\end{proposition}
\begin{proof}
...
\end{theorem}
\begin{proof}
    It suffices to prove this in the case $m = 1$ and $n = 2$. Let $\iota: A\into B$ denote the inclusion map, $S$ denote the set $B\backslash\frakq_1$ and $\varphi: B\to B_{\frakq_1}$ denote the localization map. Let $\frakP$ denote the ideal $S^{-1}(B\frakp_2)$. First, we shall show that $\iota^{-1}(\varphi^{-1}(\frakP)) = \frakp_2$.
\end{proof}
