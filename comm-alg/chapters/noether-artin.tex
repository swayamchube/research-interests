...
\begin{theorem}[Hilbert Basis Theorem]\thlabel{thm:hilbert-basis}
...
\end{corollary}
\begin{proof}
    That $R^G$ is indeed an $A$-algebra is easy to see. Further, it is well known that $R/R^G$ is an integral extension. But since $R$ is integral over $R^G$ and is also a finitely generated $R^G$-algebra, due to \thref{prop:integral-finite-type}, $R$ is a finitely generated $R^G$-module. Finally from \thref{lem:artin-tate}, $R^G$ is an $A$-algebra of finite type.
\end{proof}
...
There's an analogue of the above theorem for modules as well. 
...
\begin{lemma}\thlabel{lem:noetherian-local-property}
...
\begin{proof}
    ...
    \end{equation*}
    which is finite.
    Pick some $f\in\fraka$. If there is no $g\in\fraka$ with $S(f)\cap S(g) = \emptyset$, then, $\fraka$ is contained in $\bigcup_{i\in S(f)}\frakp_i$, and we are done due to \thref{prop:prime-containment}.
...
\begin{proof}[Proof of \thref{thm:nagata-krull-noether}]
    Note that $R_{S^{-1}\frakp_i}\cong A_{\frakp_i}$, which is noetherian, further, every element in $S^{-1}A$ is contained in finitely many of the $S^{-1}\frakp_i$'s whence, due to \thref{lem:noetherian-local-property}, $R$ is noetherian. Finally, note that the height of $S^{-1}\frakp_i$ is at least $m_{i + 1} - m_i$, consequently, $\dim R = \infty$. This completes the proof.
\end{proof}

\subsection{Eakin-Nagata Theorem}

\begin{lemma}[Formanek]\thlabel{lem:formanek}
...
\end{theorem}
\begin{proof}
    Follows from \thref{lem:formanek} by taking $M = B$.
\end{proof}
